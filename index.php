...
<form method="post" action="#">
	<div class="page-header">
     ...
     <input class="form-controlss" type="text" value="" name="price" placeholder="Please Search by price" > 
...
//$prices = ;
	...
	if (isset($_POST['btn_search']) && !empty($_POST['price'])) {
		$prices = $_POST['price'];
	$stmt = $DB_con->prepare('SELECT userID,productName, productId, price, userPic FROM tbl_users where price >='.$prices.' ORDER BY userID DESC');
	$stmt->execute();
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			?>
			<div class="col-xs-3">
				<p class="page-header"><?php echo $productName; ?></p>
...
<div class="alert alert-info">
    <strong>Coppy right</strong> <a href="#"> 2016</a>
</div>
